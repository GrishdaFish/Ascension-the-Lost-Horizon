...
def projectile(num_particles, particle_array, ox, oy, dx, dy, random_decay=False, bounce=False, color=None):
    ay = oy - dy
    ax = ox - dx
    angle = math.atan2(float(-ay), float(-ax))
    particle_array.append(Particle(ox, oy, 0.05, 1.0, angle, random_decay, bounce, color))
